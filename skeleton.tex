...
width is increased from AVX2’s 256 bits to 512 bits. AVX-512 also provides new
instructions, like integer fused-multiply-add (IFMA) (footnote IFMA-52) and
compare value into mask(VPCMPUW). 

Even though its specification was released by Intel in 2013, it had been
unpopular, as it does not bring practical performance improvements. There are
two reasons, (1) there was barely any software support, (2) huge power
consumption and overheating. For example, Intel’s implementation of AVX-512 in
the Skylake-X series consists of fusing existing 2 256 bits AVX-2 units, then
adding another FMA unit for AVX-512. This AVX-512 FMA unit results in server
thermal problems when it is built with Skylake-X’s 14 nm lithography, thus
causing throttling in clock frequency. 
